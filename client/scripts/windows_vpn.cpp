...
int main(int argc, char* argv[]) {
    ...
    // For security, only allow connection to predefined trusted VPN
    const char* allowedConnection = "HorseVPN";
    // Use RAS API to dial VPN
    RASDIALPARAMS params = {0};
    params.dwSize = sizeof(RASDIALPARAMS);
    strcpy(params.szEntryName, allowedConnection);

    // Additional security: validate that the connection exists and is properly configured
    RASENTRY entry = {0};
